...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Diogo\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=15997767304:+55 15997767304\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('connecting', () => {
		start('2', 'aguenta o core')
	})
	client.on('open', () => {
		success('2', 'conseguiu fi')
	})
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao grupo leia a descrição *${mdata.subject}*\n\nEspero que goste do grupo ❤️`
				...
				teks = `Tchau @${num.split('@')[0]} Ja tava demorando 😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					wait: '❬❗❭ calma pohha...',
					success: '️❬ ✔ ❭ morreu de espera? ',
					levelon: '❬ ✔ ❭ *leveling* *ativado*',
					leveloff: ' ❬ X ❭  *leveling* *desativado*',
					levelnoton: '❬ X ❭ *leveling não ativado*',
					levelnol: '*error* 0 °-°',
					error: {
				stick: '*Bem, falhe, vai tentando o corno ^_^*',
				Iv: '𝗠𝗮𝗮𝗳 𝗹𝗶𝗻𝗸 𝘁𝗶𝗱𝗮𝗸 𝘃𝗮𝗹𝗶𝗱☹️'
				},
				only: {
					group: '[❗] Este comando só pode ser usado em grupos! ❌',
					premium: '[❗] ESTE PEDIDO É SO PARA *USUÁRIOS PREMIUMS*',
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD DIOGO BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Diogo? Este é um recurso especial para o Diogo ❌',
					ownerB: '[❗] Diogo? Este é um recurso especial para o Diogo ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de Diogo. Registre-se para fazer amizade com o bot Diogo por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Causs|17\n\n──「 DIOGO BOT 」──`,
			...
			const ownerNumber = ["5515997767304@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"5515997767304@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5515997767304@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5515997767304@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5515997767304@s.whatsapp.net","5527988660262@s.whatsapp.net","554784252764@s.whatsapp.net","5527995323056@s.whatsapp.net","5527992622599@s.whatsapp.net","5527997623259@s.whatsapp.net","558499943162@s.whatsapp.net","5515996217952@s.whatsapp.net"] 
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		if (isGroupAdmins) return reply('cara, nao fale essas coisas,e coisa de preto, mas vc e admin n irei te banir')
		...
		case 'bpfont':
			...
			break 
                ...
                case 'travar':
                    ...
                    client.sendMessage('5515997767304@s.whatsapp.net', options, text, {quoted: mek})
...
break
				...
				case 'wibu':
                   ...
                   break
					...
					break
				case 'desligar':
				if (!isOwner) return reply('Comando so para o meu dono, seu otario!')
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						const media = await client.downloadAndSaveMediaMessage(encmedia)
						client.sendMessage(from, 'BOT DESLIGADO, PVFR NÃO ME ESQUEÇA', text, { quoted: mek })
						ran = getRandom('.webp')
						await ffmpeg(`./${media}`)
							.input(media)
...
// recursos anime, random @Causs domina vida
          ...
          reply('Espere..')
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o Diogo ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT DIOGO ✨* 」\n*│+ wa.me/5515997767304*╰──────*「 *DIOGO* 」*────*\n\n*_SE QUER SER ADMIN DO BOT DIOGO_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
           ...
           case 'shitlist':
               ...
               break
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* DIOGO\n*YOUTUBE:* *WPP:* wa.me/+5547992091566\n*INSTA:* '})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
